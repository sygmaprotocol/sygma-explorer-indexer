...
describe("Get all transfers for a specific resource between source and destination domains", function () {
  ...
  it("Should successfully fetch all ERC20TST transfers from domain 1 to domain 2", async () => {
    const res = await axios.get(
      `http://localhost:8000/api/resources/${ERC20TST_RESOURCE_ID}/domains/source/${DOMAIN_1}/destination/${DOMAIN_2}/transfers?page=1&limit=100`,
    )
  ...
  it("Should successfully fetch all permissionless generic transfers from domain 1 to domain 2", async () => {
    const res = await axios.get(
      `http://localhost:8000/api/resources/${PERMISSIONLESS_GENERIC_RESOURCE_ID}/domains/source/${DOMAIN_1}/destination/${DOMAIN_2}/transfers?page=1&limit=100`,
    )
    const transfers = res.data as Array<TransferResponse>
  ...
  it("Should successfully fetch all ERC20LRTEST transfers from domain 3 to domain 1", async () => {
    const res = await axios.get(
      `http://localhost:8000/api/resources/${ERC20LRTEST_RESOURCE_ID}/domains/source/${DOMAIN_3}/destination/${DOMAIN_1}/transfers?page=1&limit=100`,
    )
    const transfers = res.data as Array<TransferResponse>
  ...
  it("Should successfully fetch all BTC transfers from domain 4 to domain 1", async () => {
    const res = await axios.get(
      `http://localhost:8000/api/resources/${BITCOIN_RESOURCE_ID}/domains/source/${DOMAIN_4}/destination/${DOMAIN_1}/transfers?page=1&limit=100`,
    )
    const transfers = res.data as Array<TransferResponse>
    ...
    for (const transfer of transfers) {
      expect(transfer.resourceID).to.be.deep.equal(BITCOIN_RESOURCE_ID)
      expect(transfer.fromDomainId).to.be.deep.equal(parseInt(DOMAIN_4))
      expect(transfer.toDomainId).to.be.deep.equal(parseInt(DOMAIN_1))
    }
  })

  it("Should successfully fetch all BTC transfers from domain 1 to domain 4", async () => {
    const res = await axios.get(
      `http://localhost:8000/api/resources/${BITCOIN_RESOURCE_ID}/domains/source/${DOMAIN_1}/destination/${DOMAIN_4}/transfers?page=1&limit=100`,
    )
    const transfers = res.data as Array<TransferResponse>
