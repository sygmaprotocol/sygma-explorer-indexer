...
const getRpcProviderFromWebsocket = (
    rpcUrl: string, networkId: number
) => {
...
export function getProvider(rpcUrl: string, networkId: number) {
    if (rpcUrl.startsWith("wss")) {
        return getRpcProviderFromWebsocket(rpcUrl, networkId)
    } else {
        return getRpcProviderFromHttpUrl(rpcUrl)
    }
}
...
export function getHandlersMap(bridge: Domain, provider: ethers.providers.JsonRpcProvider) {
    const handlersMap: HandlersMap = {}

    bridge.handlers.forEach(h => {
        switch (h.type) {
            case "erc20":
                ...
                handlersMap[h.address] = erc20HandlerContract
                break;
            case "erc721":
                ...
                handlersMap[h.address] = erc721HandlerContract
                break;
            default:
                console.log("unsupported handler")
        }
    })
