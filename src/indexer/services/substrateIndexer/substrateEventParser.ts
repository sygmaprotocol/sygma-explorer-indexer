...
export function getSubstrateEvents(event: SygmaPalleteEvents, records: Array<SubstrateEvent>): Array<ProposalExecutionEvent | DepositEvent | FailedHandlerExecutionEvent> {
  switch (event) {
    ...
    case SygmaPalleteEvents.FailedHandlerExecution:
      return records.filter(({ event: { method, section } }: SubstrateEvent) => method === event && section === "sygmaBridge") as unknown as Array<FailedHandlerExecutionEvent>
  }
}
